...
public class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
	...
	public Void visitVarStmt(Stmt.Var stmt) {
		if (stmt.global) {
			globals.define(stmt.identifier.lexeme, evaluate(stmt.value));
		} else {
			environment.define(stmt.identifier.lexeme, evaluate(stmt.value));
		}
		return null;
	}
	...
	public Void visitIfStmt(Stmt.If stmt) {
		if (isTruthy(evaluate(stmt.condition))) {
			execute(stmt.trueBody);
		} else {
			execute(stmt.falseBody);
		}
